...
func Fetcher(config *structs.Config) {
	...
	processingChannel := make(chan map[string]any, 1000)
	outputChannel := make(chan map[string]any, 1000)
...
func parseDocuments(url string, sourceConfig structs.SourceConfig, client *http.Client, rl *rate.Limiter) []map[string]any {
...
func processWorker(processingChannel chan map[string]any, outputChannel chan map[string]any, client *http.Client, sourceRateLimit *rate.Limiter, scrapeRateLimit *rate.Limiter, requiredTermsRegexMap map[string]*regexp.Regexp, config *structs.Config, wg *sync.WaitGroup) {
...
func outputWorker(outputChannel chan map[string]any, config *structs.Config, wg *sync.WaitGroup) {
